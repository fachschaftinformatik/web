...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const checkAuthStatus = useCallback(async () => {
    ...
    try {
      ...
      setUser(data);
      if (data.theme) setPreference(data.theme as ThemePreference);
  ...
  useEffect(() => {
    if (user && user.theme) setPreference(user.theme as ThemePreference);
  }, [user, setPreference]);

  const login = (loggedInUser: User, rememberMe?: boolean) => {
    setUser(loggedInUser);
    if (loggedInUser.theme) setPreference(loggedInUser.theme as ThemePreference);
    if (rememberMe) persistRememberedUser(loggedInUser);
