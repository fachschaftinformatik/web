...
const registrationSchema = zDtoRegisterRequest
  .omit({ email: true, program_id: true })
  .extend({
    name: z.string().min(1, "Bitte gib deinen Namen ein."),
    emailPrefix: z
      .string()
      .min(1, "Bitte gib dein E-Mail ein.")
      .regex(/^[a-zA-Z0-9._-]+$/, "Die E-Mail enthält ungültige Zeichen."),
    emailDomain: z.string().min(1),
    password: z
      .string()
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      data.name &&
      data.password &&
      data.password.toLowerCase().includes(data.name.toLowerCase())
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (
      data.emailPrefix &&
      data.password &&
      data.password.toLowerCase().includes(data.emailPrefix.toLowerCase())
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export default function AuthPage() {
  ...
  useEffect(() => {
    document.title =
      tabValue === 0
        ? "Anmelden | FSV Informatik"
        : "Registrieren | FSV Informatik";
  ...
  const [emailPrefixError, setEmailPrefixError] = useState(false);
  ...
  const handleEmailPrefixChange = (
    ...
    if (value.includes("@")) {
      setEmailPrefixError(true);
    } else {
      setEmailPrefixError(false);
    }
  };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (rawData.emailPrefix.includes("@")) {
      setErrors({
        emailPrefix: "Bitte gib nur das E-Mail-Kürzel ein, ohne @-Zeichen.",
      });
      setLoading(false);
      return;
    }
  ...
  const toggleBtnStyle = {
    border:
      "1px solid " +
      alpha(theme.palette.primary.main, mode === "dark" ? 0.5 : 0.25),
    bgcolor:
      mode === "dark"
            ...
            <Stack spacing={2.5}>
              ...
              <Stack direction="row" spacing={1}>
                <TextField
                  ...
                  disabled={loading || (tabValue === 1 && !!success)}
                  error={emailPrefixError || !!errors.emailPrefix}
                  helperText={
                    emailPrefixError
                      ? "Bitte gib das E-Mail-Kürzel ohne @-Zeichen ein."
                      : errors.emailPrefix
                  }
                  value={emailPrefix}
                ...
                <TextField
                  ...
                  name="emailDomain"
                  id="emailDomain"
                  defaultValue={ALLOWED_DOMAINS[0]}
                  disabled={loading || (tabValue === 1 && !!success)}
                  sx={{ minWidth: 160 }}
                >
